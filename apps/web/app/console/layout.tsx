...
import { BendPage } from "@/components/console/bend";
 ...
 *
 * ## The console scrolls on the face of a cube
 *
 * `BendPage` wraps all of it, header included, because the effect is one face
 * rather than a decoration applied section by section: the top and bottom of
 * the whole screen fold away over a virtual edge as it scrolls, and flatten
 * back out at each end. Wrapping here rather than per screen is the argument
 * the gate below already makes — a screen added next month is on the same face
 * without anyone remembering to put it there.
 *
 * It brings the scrolling with it. This region is the console's scroll
 * container from here on and the document no longer scrolls at all; see
 * `components/console/bend.tsx` for what that costs and what pays it back.
 */
...
export default function ConsoleLayout({ children }: LayoutProps<"/console">) {
  return (
    ...
    */
    <BendPage>
      <VStack width="100%" minHeight="100%" className="surface-body">
        {/*
