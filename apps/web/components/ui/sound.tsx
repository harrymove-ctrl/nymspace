...
/*
  Kobra's own components announce themselves with `data-slot`; this app is
  built on Astryx, which announces itself with roles and `data-variant`
  (`themeProps()` in @astryxdesign/core reflects every visual prop as a data
  attribute). So the roles Astryx actually renders are added here — combobox
  and listbox options from `Selector`, radios from `SegmentedControl`, switches
  — or those controls fall through to the nearest ancestor button and answer
  with the wrong cue, or none.
*/
const INTERACTIVE =
  '[data-slot], button, a[href], [role="button"], [role="option"], [role="menuitem"], [role="combobox"], [role="radio"], [role="switch"], [role="tab"], input[type="checkbox"], input[type="radio"]'

/*
  What an arriving element has to say to be heard as a result. `false` is
  excluded on purpose: React renders `data-success={false}` as the string
  "false" rather than omitting it, so a component that always writes the
  attribute and flips its value would otherwise announce every render.
*/
const ANNOUNCE_SUCCESS = '[data-success]:not([data-success="false"])'
const ANNOUNCE_ERROR = '[data-error]:not([data-error="false"]), [aria-invalid="true"]'
...
function classify(el: HTMLElement): Cue | null {
  ...
  /*
    A choice among siblings, whatever renders it. `SegmentedControl` paints
    `role="radio"` and `Selector` paints `role="option"`; both are the same
    gesture as a menu item, and the row pitch is what makes a list of them
    sound like a list rather than one note repeated.
  */
  if (el.matches('[role="option"], [role="radio"], [role="tab"]')) {
    return { sound: 'select', detune: rowPitch(el) }
  }

  /*
    Disclosure, before the button fallback.

    The rule above this one reaches open/close only through a `-trigger` slot,
    which no Astryx component sets. `aria-expanded` is the same fact stated the
    way the platform states it, and it is what `Collapsible` and `Selector`
    actually render — so a panel opening answers with `open` rather than the
    flat `tap` every other button gets.
  */
  const expanded = el.getAttribute('aria-expanded')
  if (expanded !== null) return { sound: expanded === 'true' ? 'close' : 'open' }
...
function SoundEffectListener() {
  ...
  useEffect(() => {
    ...
    /*
      A result that arrives as a new element, not as a changed one.

      The attribute half of this observer only ever hears about nodes that were
      already on the page when they flipped. Every outcome in this console is a
      fresh `Banner` React mounts when the write comes back — the node does not
      change state, it appears already carrying it — so an outcome would have
      been silent while a checkbox two rows up was not.

      Flags rather than immediate plays: one batch can insert forty rows, and
      forty banners are still one thing happening.
    */
    const announce = (root: Node, seen: { success: boolean; error: boolean }) => {
      if (!(root instanceof HTMLElement)) return
      const matches = (selector: string) =>
        root.matches(selector) || root.querySelector(selector) !== null
      if (!seen.success && matches(ANNOUNCE_SUCCESS)) seen.success = true
      if (!seen.error && matches(ANNOUNCE_ERROR)) seen.error = true
    }

    const observer = new MutationObserver((records) => {
      let slid = false
      const arrived = { success: false, error: false }

      for (const record of records) {
        if (record.type === 'childList') {
          for (const node of record.addedNodes) announce(node, arrived)
          continue
        }
        ...
        switch (record.attributeName) {
          ...
          case 'data-success': {
            if (entered) arrived.success = true
            break
          }
          /*
            `data-error` beside `aria-invalid`, because a refused payment is not
            an invalid field. `aria-invalid` belongs on a form control and says
            "what you entered is wrong"; the console's faults are banners
            reporting that a write failed, and putting `aria-invalid` on one
            would be telling a screen reader the banner needs correcting.
          */
          case 'data-error':
          case 'aria-invalid': {
            if (entered) arrived.error = true
            break
          }
          default:
            break
      ...
      /*
        Success first, and at most one of each. Both firing in one batch means
        a screen that reports a partial result — the pair reads as "done, with
        something to look at", which is what happened.
      */
      if (arrived.success) play(patch, { sound: 'success' })
      if (arrived.error) play(patch, { sound: 'error' })
    })
    ...
    observer.observe(document.body, {
      ...
      attributeFilter: ['aria-valuenow', 'aria-invalid', 'data-success', 'data-error'],
    })
...
export function SoundToggle({ className, onClick, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      {...props}
      className={cn(
        // `text-shell-fg-faint` / `text-shell-fg-muted` came from Kobra's own
        // theme and resolve to nothing here, so the icon inherited body ink and
        // sat louder than the theme toggle beside it. These are the tokens
        // `app/globals.css` actually declares, matched to `ThemeToggle`.
