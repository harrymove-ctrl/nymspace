/**
 * The page as the face of a cube.
 *
 * Cloned from `canvas-ui` (`src/lib/Bend/BendVanilla.ts`, MIT, David Haz) and
 * changed in exactly one place: where the texture comes from.
 *
 * Upstream reads its texture with `drawElementImage`, Chromium's experimental
 * html-in-canvas call, which is behind a flag and ships nowhere. The shader
 * below — the fold, the arc that rounds the crease, the perspective divide, the
 * overscroll tumble — is plain WebGL2 and runs everywhere. So the Chromium-only
 * part is the supplier of a texture, not a participant in the effect, and this
 * file takes the second supplier the console already has: `lib/dom-raster.ts`,
 * written for DecryptReveal for this same reason. Same trade, same file, second
 * caller.
 *
 * Two consequences follow from the raster being a silhouette rather than a
 * screenshot, and both are handled in `render`:
 *
 * - The bent face only replaces the live DOM while the fold is actually doing
 *   something. A region short enough not to scroll never folds, so it is never
 *   covered and never rasterised — it is the untouched DOM, pixel for pixel.
 * - Pointer mapping, hover forwarding and caret hit-testing switch on with the
 *   cover, not on `htmlInCanvas`. A click lands where the operator saw the
 *   thing they clicked, which on a folded edge is not where the DOM has it.
 */

import { createDomRaster, type DomRaster } from "../dom-raster";
...
export function createBend(
  ...
  /** The same colour as `bg`, as CSS. The raster fills with it. */
  let bgCss = "#000";
  ...
  function syncCanvasSize() {
    ...
    if (!htmlInCanvas) contentDirty = true;
    if (htmlInCanvas) {
  ...
  /**
   * Whether the region has anything to scroll.
   *
   * The gate on the whole raster path. A frame whose content fits never folds,
   * so covering it with a raster would trade real pixels for approximate ones
   * and buy no effect at all.
   */
  ...
  */
  ...
  let rasterBg = bgCss;
  /** Is the output canvas showing instead of the DOM, right now? */
  let covered = htmlInCanvas;
  let hoverRulesPatched = false;
  ...
  function uploadContent(engaged: boolean) {
    ...
    if (!raster || !engaged) {
      // Cleared even though nothing was uploaded: `frame` treats a dirty
      // texture as a reason to keep running, and an uncovered region would
      // never stop.
      contentDirty = false;
      return;
    }
    /*
      Repainted every frame the fold is moving rather than only when something
      marked itself dirty. The alternative is a dirty flag that has to be right
      about CSS transitions, row-enter animations and anything else that repaints
      without mutating the tree — a list that grows and whose failure mode is a
      frozen texture under a moving fold. The raster only runs while the effect
      is running, which is the window in which it has to be current.
    */
    contentDirty = false;
    syncBgColor();
    if (bgCss !== rasterBg) {
  ...
  /**
   * Show the bent face, or get out of the way.
   *
   * In raster mode the DOM stays in the document and keeps receiving events —
   * `opacity: 0` hides ink, not hit-testing — because the pointer forwarding
   * below re-dispatches through it. `visibility` or `display` would take the
   * events with the pixels.
   */
  function setCovered(next: boolean) {
    ...
    if (next && !hoverRulesPatched) {
      hoverRulesPatched = true;
      patchHoverRules();
      content.setAttribute(CONTENT_ATTR, "");
    }
    if (!next) setHoverTarget(null);
  }

  function render() {
    /*
      The fold is doing something, so the DOM underneath is in the wrong place.
      Nothing folds without somewhere to scroll, which is why `scrollable` gates
      this rather than the fold amounts alone: the tilt and the tumble are
  ...
  function onWheel(event: WheelEvent) {
    ...
    if (!htmlInCanvas && !scrollable) return;
    const max = content.scrollHeight - content.clientHeight;
    if (max <= 1) return;
  ...
  function onPointerMove(event: PointerEvent) {
    ...
    if (config.tilt > 0 && !reducedMotion && (htmlInCanvas || scrollable)) {
  ...
  */
  ...
  // `relatedTarget` is the element about to receive focus. Reading
  ...
  function onSelectionChange() {
    ...
    if (next === selectionWithin) return;
    selectionWithin = next;
    start();
  }
  ...
  /*
    The raster has no `onpaint` to tell it the subtree changed, so a mutation
    wakes the loop; the loop then repaints every frame until it settles. Scoped
    to `content`, so it observes one frame's worth of console rather than the
    document.
  */
  const mutations = htmlInCanvas
    ? null
  ...
  return {
    ...
    destroy() {
      ...
      if (!htmlInCanvas) content.style.opacity = "";
