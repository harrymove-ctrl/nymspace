/**
 * DecryptReveal — the cipher engine, vendored.
 *
 * Upstream is `src/lib/DecryptReveal/DecryptRevealVanilla.ts` in
 * github.com/DavidHDev/canvas-ui, a copy-in registry rather than a published
 * package: its components are meant to be pasted into a project, which is why
 * this is a file in `lib/` and not a dependency in `package.json`. Vendored
 * verbatim apart from the changes recorded below, so a future upstream diff
 * is readable.
 *
 * One thing to settle before this ships anywhere it matters: **the upstream
 * repository carries no LICENSE file**, so GitHub reports it as
 * `NOASSERTION` and there is no grant here to rely on. A copy-in registry
 * plainly intends to be copied, which is why this is in the tree at all, but
 * intent is not a licence. Ask the author, or drop this file and keep
 * `lib/cipher-field.ts`, which owes it nothing.
 *
 * The changes made to the copy:
 *
 *  1. `createRectCache` is inlined below. Upstream imports it from a sibling
 *     module shared with its other effects; this is the only caller here, and
 *     a second file for twenty lines is worse than the import it saves.
 *  2. A `capture` hook on {@link DecryptRevealElements}, and the four call
 *     sites that read `canCapture` where upstream reads `htmlInCanvas`. This
 ...
 *
 * Everything that makes this DecryptReveal — matching each cell's ink to the
 * ASCII glyph whose shape fits, the decrypt circle, the flicker band, the
 * chromatic aberration at its edge — is WebGL2, and WebGL2 has been everywhere
 * for years. None of it asks the browser for anything unusual.
 *
 * Exactly one line does: `drawElementImage`, the experimental html-in-canvas
 * call that turns a live subtree into pixels, paired with `requestPaint`. That
 * is Chromium-behind-a-flag, and upstream uses it for one job — handing the
 * shader a picture of the UI. It supplies the texture; it is not part of the
 * effect.
 *
 * So `capture` lets a caller supply that texture instead, and
 * `lib/dom-raster.ts` supplies it by repainting the subtree with Canvas2D. With
 * it, this file runs the real effect in Firefox, in Safari, and in a Chrome
 * with no flags set. {@link supportsHtmlInCanvas} still answers whether the
 * native path is available, and is now a choice between two suppliers rather
 * than a check for whether the thing works at all.
 *
 * A caller that supplies neither gets upstream's own degradation: `uCrisp`
 * goes high and the content is drawn with no cipher over it. Anything using
 * this to *withhold* a UI must not rely on the veil for that —
 * `components/console/decrypt-gate.tsx` is the caller, and its `inert` is the
 * part that is not an effect.
 */


/**
 * The container's viewport rect, kept current without a per-frame read.
 *
 * Inlined from upstream's shared `rect-cache` module (see the file header).
 * `getBoundingClientRect` forces layout, and the pointer handler runs on every
 * mouse move; the observers refresh it only when something could actually have
 * moved it.
 */
...
export interface DecryptRevealElements {
  /**
   * Canvas with layoutsubtree that hosts the HTML content, under
   * html-in-canvas. When {@link DecryptRevealElements.capture} is supplied this
   * is unused and may be any detached canvas.
   */
  ...
  /**
   * Supplies the content texture without html-in-canvas — the one addition to
   * the vendored engine, and the one that makes it run outside Chromium (see
   * the file header, change 3).
   *
   * Called whenever the texture is stale, with the device pixel ratio the
   * output is rendered at. It must return a canvas the size of the content, or
   * null when there is nothing to draw yet; everything downstream of it is
   * upstream's own code, unmodified. `lib/dom-raster.ts` is the implementation.
   */
...
export function createDecryptReveal(
  ...
  /*
    Whether a texture can be produced at all, by either route. Upstream reads
    `htmlInCanvas` at each of these sites; the rename is so a reader can see
    that the question being asked is "is there a picture of the UI?" and not
    "is this Chromium?". The distinction is the whole point of the `capture`
    hook — see the file header.
  */
  const canCapture = htmlInCanvas || typeof capture === "function";
  ...
  function syncCanvasSize() {
    ...
    if (htmlInCanvas) {
      ...
      // No `onpaint` to wait for: the raster is redrawn on demand, so a resize
      // only has to say that what is uploaded is now the wrong size.
      contentDirty = true;
    }
    cellsDirty = true;
  }
  ...
  function uploadContent() {
    ...
    /*
      Upstream uploads `source`, the canvas html-in-canvas painted into. The
      `capture` hook returns a canvas of its own instead, and from here down
      the two are the same thing: a canvas-shaped texture source, which is what
      `texImage2D` wanted all along.
    */
    let texture: HTMLCanvasElement = source;
    if (!htmlInCanvas) {
      const dpr = output.width / Math.max(output.clientWidth, 1);
      const painted = capture!(dpr);
      // Nothing to draw yet — leave the flag up so the next frame tries again
      // rather than rendering a cipher of a blank texture.
      if (!painted) return;
      texture = painted;
    }
  ...
  function render() {
    ...
    /*
      `uCrisp` draws the sampled UI with no cipher over it at all, which is
      upstream's answer to both "there is no texture" and "this reader asked
      for reduced motion".

      Only the first of those survives here. This engine is the console's
      connect gate, and a reader who prefers reduced motion is asking for less
      movement, not for the locked screen to open — lifting the veil for them
      would be the accessibility setting deciding an access question. Reduced
      motion still stops the easing and the scramble, which it does above; what
      it no longer does is turn the cipher off. See `decrypt-gate.tsx`.
    */
